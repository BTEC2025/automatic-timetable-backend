...
export async function POST(req: Request) {
    try {
        ...
        const teacherCodes = Array.from(
            new Set(rows.map((row) => pick(row, FIELD_ALIASES.teacherId)).filter(Boolean))
        );
        ...
        const [teachers, subjects] = await Promise.all([
            Teacher.find({ teacher_id: { $in: teacherCodes } }).select("teacher_id").lean(),
            Subject.find({ subject_id: { $in: subjectCodes } }).select("subject_id").lean()
        ]);

        const teacherSet = new Set(teachers.map((teacher) => teacher.teacher_id));
        const subjectSet = new Set(subjects.map((subject) => subject.subject_id));

        const existingTeach = await Teach.find({
            teacher_id: { $in: teacherCodes },
        ...
        const existingPairSet = new Set(
            existingTeach.map((entry) => `${entry.teacher_id}::${entry.subject_id}`)
        );
        ...
        for (let index = 0; index < rows.length; index += 1) {
            ...
            try {
                ...
                if (!teacherId || !subjectId) {
                    throw new Error("teacher_id and subject_id are required");
                }
                ...
                if (existingPairSet.has(pairKey)) {
                    throw new Error(`Teach entry already exists for teacher ${teacherId} and subject ${subjectId}`);
                }
        ...
        console.error("IMPORT TEACH ERROR:", err);
