...
const FIELD_ALIASES = {
    roomId: ["room_id", "roomid", "รหัสห้อง"],
    roomName: ["room_name", "ชื่อห้อง", "room"],
    roomType: ["room_type", "ประเภท", "type"]
};
...
export async function POST(req: Request) {
    try {
        ...
        const roomIds = rows.map((row) => pick(row, FIELD_ALIASES.roomId)).filter(Boolean);
        const existing = await Room.find({ room_id: { $in: roomIds } }).select("room_id").lean();
        const existingSet = new Set(existing.map((room) => room.room_id));
        ...
        for (let index = 0; index < rows.length; index += 1) {
            ...
            try {
                const roomId = pick(row, FIELD_ALIASES.roomId);
                const roomName = pick(row, FIELD_ALIASES.roomName);
                const roomTypeRaw = pick(row, FIELD_ALIASES.roomType).toLowerCase();
                const roomType = roomTypeRaw === "practice" ? "practice" : "theory";

                if (!roomId || !roomName) {
                    throw new Error("room_id and room_name are required");
                }
        ...
        console.error("IMPORT ROOM ERROR:", err);
